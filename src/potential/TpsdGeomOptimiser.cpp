...
//#define TPSD_GEOM_OPTIMISER_CONV
...
const double TpsdGeomOptimiser::DEFAULT_MAX_DELTA_POS_FACTOR = 0.8;
const double TpsdGeomOptimiser::DEFAULT_MAX_DELTA_LATTICE_FACTOR = 0.3;
const int TpsdGeomOptimiser::MAX_FAILED_POTENTIAL_EVALUATIONS = 10;
    ...
    const OptimisationSettings & settings) const
{
  ...
  for(iter = 0; !converged && iter < *settings.maxIter; ++iter)
  {
    ...
    if(!evaluator.evalPotential().second)
    {
      ...
      if(++numLastEvaluationsWithProblem > MAX_FAILED_POTENTIAL_EVALUATIONS)
      {
        outcome = OptimisationOutcome::failure(
            "Failed to evaluate potential too many times.");
        break;
      }
    }
    else
  ...
  std::cout << "Optimisation took: " << static_cast< float>(t) / CLOCKS_PER_SEC
  << std::endl;
#endif
...
#ifdef TPSD_GEOM_OPTIMISER_CONV
    std::cout << h << std::endl;
#endif
