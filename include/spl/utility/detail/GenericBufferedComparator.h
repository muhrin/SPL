...
template< class ComparatorTyp>
  GenericBufferedComparator< ComparatorTyp>::GenericBufferedComparator(
      const ComparatorTyp & comparator) :
      myLastHandleId(0), myComparator(comparator), myTotalData(0)
  {
  }

template< class ComparatorTyp>
  double
  GenericBufferedComparator< ComparatorTyp>::compareStructures(
      const ComparisonDataHandle & str1, const ComparisonDataHandle & str2)
  {
    return myComparator.compareStructures(getComparisonData(str1.getId()),
        getComparisonData(str2.getId()));
  }

template< class ComparatorTyp>
  bool
  GenericBufferedComparator< ComparatorTyp>::areSimilar(
      const ComparisonDataHandle & str1, const ComparisonDataHandle & str2)
  {
    return myComparator.areSimilar(getComparisonData(str1.getId()),
        getComparisonData(str2.getId()));
  }
...
template< class ComparatorTyp>
      ...
      const spl::common::Structure & structure)
  {
    ...
    myComparisonData.insert(id,
        myComparator.generateComparisonData(structure).release());
    return handle;
  }

template< class ComparatorTyp>
  const IStructureComparator &
  GenericBufferedComparator< ComparatorTyp>::getComparator() const
  {
    return myComparator;
  }
...
template< class ComparatorTyp>
  ...
  GenericBufferedComparator< ComparatorTyp>::getComparisonData(
      const HandleId & id)
  {
    const typename DataMap::const_iterator it = myComparisonData.find(id);

    SSLIB_ASSERT_MSG(it != myComparisonData.end(),
...
template< class ComparatorTyp>
  void
  GenericBufferedComparator< ComparatorTyp>::handleReleased(const HandleId & id)
  {
